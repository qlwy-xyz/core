...
async function main() {
  ...
  console.log(
    `Setting cast fee on ${coreAddress} (network ${connection.networkName}) to castFee=${castFee}`
  );

  const txHash = await walletClient.writeContract({
    abi: CORE_ABI,
    address: coreAddress,
    functionName: "setCastFee",
    args: [BigInt(castFee)],
    account,
    chain,
  });
  ...
  if (receipt.status !== "success") {
    throw new Error(`Set cast fee failed with status ${receipt.status}`);
  }

  console.log(`Cast fee updated in tx ${receipt.transactionHash}`);
}
